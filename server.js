...
// set the view engine to ejs (WAS PUG)
...
app.get('/player_info/select_player', function(req, res) {
	var x = req.query.player_choice
	var query1 = 'select name,id from football_players;'
	var query2 = 'select * from football_players WHERE id='+x+';'
	var query3 = 'select count(*) from football_players WHERE id='+x+';'
	var query4 = 'SELECT COUNT(*) FROM football_games FULL JOIN football_players ON football_players.id = any(football_games.players) WHERE football_players.id ='+x+';'
	db.task('get-everything', task => {
		return task.batch([
			task.any(query1),
			task.any(query2),
